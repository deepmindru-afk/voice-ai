...
func (a *adapter) GetChatCompletion(
	...
	if err != nil {
		a.logger.Errorf("custom-llm openai_compatible: request failed: %v", err)
...
func (a *adapter) StreamChatCompletion(
	...
	if resp.Err() != nil {
		err := resp.Err()
		a.logger.Errorf("custom-llm openai_compatible: stream init failed: %v", err)
		failure := metrics.OnFailure().Build()
		...
		for i, choice := range chatCompletions.Choices {
			...
			if onStream != nil {
				if err := onStream(options.Request.GetRequestId(), tokenMsg); err != nil {
					a.logger.Warnf("custom-llm openai_compatible: onStream error: %v", err)
				}
			}
		}
	}
	...
	if resp.Err() != nil {
		err := resp.Err()
		a.logger.Errorf("custom-llm openai_compatible: stream read failed: %v", err)
		failure := metrics.OnFailure().Build()
		options.PostHook(map[string]interface{}{
			"error":  err,
			"result": utils.ToJson(resp),
...
func (a *adapter) applyChatCompletionParameters(
	...
	for key, value := range params {
		...
		case "tool_choice":
			...
			default:
				a.logger.Warnf("custom-llm openai_compatible: unknown tool_choice %q", choice)
			}
