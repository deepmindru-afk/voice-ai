...
func NewStream(logger commons.Logger, credential *protos.Credential) (internal_callers.ChatStream, error) {
	if _, err := internal_custom_llm_common.ParseClientConfig(logger, credential); err != nil {
		logger.Errorf("custom-llm openai_chat_completions: failed to create stream client: %v", err)
		return nil, err
	}
...
func (s *streamCaller) Connect(ctx context.Context, configuration *protos.StreamChatConfiguration) error {
	...
	if err != nil {
		s.logger.Errorf("custom-llm openai_chat_completions: failed to create stream client: %v", err)
		return err
	}
	s.client = client
...
func (s *streamCaller) Chat(
	...
	if resp.Err() != nil {
		err := resp.Err()
		s.logger.Errorf("custom-llm openai_chat_completions: stream init failed: %v", err)
		...
		for i, choice := range chatCompletions.Choices {
			...
			if onStream != nil {
				if err := onStream(requestID, tokenMsg); err != nil {
					s.logger.Warnf("custom-llm openai_chat_completions: onStream error: %v", err)
				}
			}
		}
	}
	...
	if resp.Err() != nil {
		err := resp.Err()
		s.logger.Errorf("custom-llm openai_chat_completions: stream read failed: %v", err)
		failure := metrics.OnFailure().Build()
		if options.PostHook != nil {
			options.PostHook(map[string]interface{}{
				"error":  err,
