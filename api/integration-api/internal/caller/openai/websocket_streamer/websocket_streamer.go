...
func New(logger commons.Logger, credential *protos.Credential) (internal_callers.ChatStream, error) {
	if _, err := internal_openai_common.ResolveAPIKey(credential); err != nil {
		logger.Errorf("Failed to create OpenAI websocket stream client: %v", err)
		return nil, err
	}
...
func (s *streamer) Connect(ctx context.Context, configuration *protos.StreamChatConfiguration) error {
	...
	if err != nil {
		s.logger.Errorf("Failed to create OpenAI websocket stream client: %v", err)
		return err
	}
...
func (s *streamer) Chat(
	...
	s.logger.Benchmark("Openai.websocket_streamer.Chat.llmRequestPrepare", time.Since(start))
