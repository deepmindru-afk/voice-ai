...
func (n *ArgumentNamespace) Get(suffix string, src variable.Source, _ variable.ResolveContext) (any, bool) {
	v, ok := src.Arguments()[suffix]
	return v, ok
}

func (n *ArgumentNamespace) Enumerate(src variable.Source, _ variable.ResolveContext) map[string]any {
	in := src.Arguments()
