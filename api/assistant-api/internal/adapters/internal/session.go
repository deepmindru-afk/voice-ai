...
// Connect starts dispatchers and enqueues the init chain. It always returns nil
// because initialization runs asynchronously on the low dispatcher goroutine.
// The gRPC stream is already open by the time Connect is called; any init errors
// are delivered to the client via InitializationErrorPacket → ConversationError
...
func (r *genericRequestor) Connect(ctx context.Context, auth types.SimplePrinciple, config *protos.ConversationInitialization) error {
	...
	r.OnPacket(ctx, internal_type.InitAssistantPacket{
		ContextID: r.GetID(),
		Config:    config,
	})
	return nil
...
// handleDisconnectShutdown — the terminal step of the disconnect chain.
...
func (r *genericRequestor) Disconnect(ctx context.Context) {
	...
	r.OnPacket(ctx, internal_type.DisconnectCloseIOPacket{ContextID: r.GetID()})
	select {
	case <-done:
